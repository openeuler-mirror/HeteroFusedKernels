...
def benchmark():
    ...
    for bs in range(8, 2048, 8):
        bs *= step
        run_count = 1000    
        input_ids = torch.randint(0, embed_size, [run_count, bs*10], dtype=torch.int64).npu()
        embedDst = torch.empty([bs*10, embed_dim], dtype=dtype).npu()
        ...
        print("Kernel test {} time: {} - bandwidth: {}{}".format(bs, (b - a) / run_count * 1000, bs * 10 * run_count * embed_dim * 2/ ((b - a) * 1000000000),ok)) 
